...
//    //设置端口
//    des_addr.sin_port = htons(_des_port.text.intValue);
//    //设置IP地址
//    des_addr.sin_addr.s_addr = inet_addr(_des_ipAdress.text.UTF8String);
//    //连接
//
...
//
//            if (recvLen > 0) {
//                NSString *recvStr = [NSString stringWithFormat:@"[UDP消息][来自服务端%@：%@]：%@", _dest_ipAdr, _dest_port, [NSString stringWithUTF8String:buf]];
//                NSLog(@"%@",recvStr);
//                dispatch_async(dispatch_get_main_queue(), ^{
//                    self->_recvTextView.text = recvStr;
//                });
...
//        des_addr.sin_family      = AF_INET;
//        des_addr.sin_port        = htons(_dest_ipAdr.intValue);
//        des_addr.sin_addr.s_addr = inet_addr(_dest_port.UTF8String);
//        // 发送数据
//        //NSLog(String(des_addr.sin_port));
...
//        //NSLog(@"%@", des_addr.sin_port);
//        NSLog(@"%@", _sendTF);
//        NSLog(@"%d", errno);
//        //NSLog(@"%d", GetLastError());
//
//    }
//    if (sendLen > 0) {
//        NSLog(@"发送成功");
